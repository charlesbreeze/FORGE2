...
=head1 SYNOPSIS
Provise plotting utilities for different plots to Forge2
=head1 EXPORT
Chart
dChart
table
...
d1 <- dPlot(
  y = 'log10pvalue',
  x = c('TissueCell'),
  groups = c('TissueCell', 'Accession', 'Pvalue', 'Qvalue', 'SNP'),
  data = results,
  type = 'bubble',
  width = dplot.width,
  height = dplot.height,
...
d1\$setTemplate(afterScript = paste0(\"
  <script>
    ...
    myChart.svg.append('text')
      ...
      .text('SNPs in Histone mark (broadPeak) sites in cell lines for $data $label')
    \", labels.string, \"
    \", lines.string, \"
...
sub table{
    ...
    print $rcfh "setwd('$Rdir')
results <- read.table('$filename', header = TRUE, sep='\\t')
results <- subset(results, T, select = c('Cell', 'Tissue', 'Accession', 'Pvalue', 'Qvalue', 'SNP'))
require(rCharts)
dt <- dTable(
    results,
    sScrollY = '600',
