...
use Exporter;
@ISA = qw(Exporter);
@EXPORT = qw(get_all_datasets get_all_arrays get_all_proximity_filters process_file get_random_matching_picks process_overlaps get_probe_annotations_and_overlap_for_dataset get_samples_from_dataset assign save_probe_annotation_stats proximity_filter ld_filter);
...
=head2 save_probe_annotation_stats
 ...
 Description    : Save stats about the probe annotations on a text file. The $set_id is either
                  ...
                  The file will contain for each set the whole list of MAF, TSS distance and GC content
...
sub save_probe_annotation_stats {
    ...
    say STATS join("\t", $flag, "maf", @maf);
    say STATS join("\t", $flag, "tss", @tss);
    say STATS join("\t", $flag, "gc", @gc);
    close(STATS);
}
...
sub get_random_matching_picks {
    ...
    # load up the stored hashes that contain the bins of dmps by MAF, TSS distance and GC content.
    my %bins;
    ...
    my $params_file = $datadir . "/dmp_${array}_params";
    my %params = %{ retrieve($params_file)};

    foreach my $probe_id (keys %{$overlaps->{'DMPS'}}) {
        my ($maf_feature, $tss_feature, $gc_feature) = split("\t", join("\t", $overlaps->{'DMPS'}->{$probe_id}->{'PARAMS'}));
...
sub process_overlaps {
    ...
    foreach my $row (@{$rows}){
        my ($probeid, $sum, $bit_string, $maf_feature, $tss_feature, $gc_feature) = @$row;
        $overlaps->{'DMPS'}->{$probeid}->{'SUM'} = $sum;
        $overlaps->{'DMPS'}->{$probeid}->{'PARAMS'} = join("\t", $maf_feature, $tss_feature, $gc_feature);
...
sub get_probe_annotations_and_overlap_for_dataset {
    ...
    for (my $loop = 0; $loop * $MAX_SQL_VARIABLES < @$probe_ids; $loop++) {
        ...
        my $sql = "SELECT probe_id, sum, bit, minor_allele_freq, distance_to_tss, gc_content
